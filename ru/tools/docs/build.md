...
Сборку больших проектов можно ускорить, обрабатывая страницы параллельно, - см. [Многопоточная сборка](multithreading.md).
...
При запуске с ключом [`--build-stats`](settings.md#build-stats-flag) (или `buildStats: true` в [файле конфигурации](../../settings.md)) рядом с output записывается файл `yfm-build-stats.json` с метриками текущей сборки. Файл предназначен для CI-дашбордов, отслеживания регрессий и диагностики — для рантайма он не нужен.
...
При запуске с ключом [`--build-content`](settings.md#build-content-flag) (или `buildContent: true` в [файле конфигурации](../../settings.md)) рядом с output записывается файл `yfm-build-content.json` с отпечатком содержимого каждой страницы и ассета. Файл предназначен для внешних инструментов, которые сравнивают две сборки и точно определяют список изменившихся страниц — переиндексация поиска, уведомления об изменениях и т.п. Для рантайма он не нужен.
