# CLI

##[@diplodoc/cli](https://www.npmjs.com/package/@diplodoc/cli)## — пакет для сборки документационного проекта с навигацией, внутренними переходами и полной поддержкой YFM.

Собранный проект представляет собой набор статических HTML, которые можно просмотреть локально или разместить на хостинге, в GitHub Pages или [S3](publish-s3.md).

## Установка {#install}

Чтобы установить пакет, выполните команду:

```shell
npm i @diplodoc/cli -g
```

## Команды

- [создание проекта](./init.md);
- [сборка](./build.md);
- [сборка одного файла](./content.md);
- [перевод](./translate.md).
