...
Общие опции команды (`--source`, `--target`, `--files`, `--include`, `--exclude`, `--dry-run` и другие) описаны на странице [Локализация](translate.md). Опция `--target` может быть передана несколько раз - перевод выполнится на каждый язык. Ниже - опции AI-провайдеров.
