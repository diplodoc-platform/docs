...
Каждый документ разбивается на сегменты - предложения, заголовки, ячейки таблиц. Разметка YFM, HTML-теги, Liquid-конструкции и код (кроме комментариев и подписей, см. [Код и диаграммы](#code)) на перевод не отправляются: они остаются в «скелете» документа, и после перевода сегменты подставляются обратно на свои места. Повторяющиеся сегменты переводятся один раз.
...
### Код и диаграммы {#code}

Из блоков кода на перевод уходит только то, что написано для читателя. Сколько именно, задает опция `--code` (в секции `translate` файла `.yfm` - ключ `code`):

#|
|| **Режим** | **Что переводится** ||
|| `no` | Ничего, блоки кода копируются как есть ||
|| `precise` | Плейсхолдеры в угловых скобках (`<cluster-name>`) и комментарии в блоках `bash` и `shell` ||
|| `adaptive` | Дополнительно строчные комментарии в блоках любого языка (`#`, `//`, `--`) и подписи диаграмм [Mermaid](../../custom-plugins/mermaid.md): узлы, стрелки, заметки, заголовки. Закомментированный код, директивы инструментов и разделители остаются как есть ||
|| `all` | Блок целиком, включая код ||
|#

По умолчанию AI-провайдеры работают в режиме `adaptive`, [Yandex Translate](translate-yandex.md) - в режиме `precise`. Сам код не переводится ни в одном режиме, кроме `all`.

Режим отдельного блока задается в его информационной строке:

````markdown
```yaml translate=precise
# Этот комментарий останется на исходном языке
key: value
```
````

Подкоманда [`seed`](translate-ai.md#seed) должна работать в том же режиме, что и перевод: от режима зависит, на какие сегменты разбивается файл. Если меняете режим, передайте одинаковое значение `--code` обеим командам.
...
||
|| `--code` |
Сколько из блоков кода уходит на перевод: `no`, `precise`, `adaptive` или `all`. По умолчанию - `adaptive` для AI-провайдеров и `precise` для Yandex Translate. См. [Код и диаграммы](#code)
||
|| `--vars`, `-v` |
...
|| `cache` | `enabled` - был ли включен кэш, `hits` и `misses` - обращения, `hitRate` - доля попаданий или `null`, `hints` - сегменты, отправленные с прежней версией из сида (см. [Измененные предложения](translate-ai.md#seed-hints)) ||
