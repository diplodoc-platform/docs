# Параметры сборки документации

Ниже перечислены параметры сборки, поддерживаемые при выполнении команды `yfm build`.

#|
||
**Ключ запуска** | **Описание**
||
||
`--input, -i` | Путь до директории проекта. По умолчанию – текущая директория.
||
||
`--output, -o` | Путь до директории для выходных данных (обязательный параметр).
||
||
`--vars-preset <name>` {#vars-preset} | Название используемого [пресета переменных](../../project/presets.md).
||
||
`--vars, -v` | Значения [переменных](../../syntax/vars.md).
||
||
`--strict, -s`
|
Запуск в строгом режиме.

Предупреждения YFM трактуются как ошибки. По умолчанию выключено.
||
||
`--quiet, -q`
|
Запуск в тихом режиме.

Не выводить логи в stdout. По умолчанию выключено.
||
||
`--jobs, -j` {#jobs}
|
Запуск в [многопоточном режиме](multithreading.md).
Если не указать конкретное число потоков, оно вычислится автоматически исходя из параметров системы

`--jobs 4`, `-j4`, `-j`
||
||
`--worker-max-old-space` {#worker-max-old-space}
|
Лимит памяти (old space) для каждого рабочего потока, в мегабайтах. Учитывается только при запуске с `--jobs`.

`--worker-max-old-space 4096`
||
||
`--config, -c` {#config} | Путь до [файла конфигурации](../../settings.md).
||
||
`--extensions, -e`
|
Подключение внешних расширений при запуске.

Передача пути отличается для exec-флагов и конфигурации .yfm. Для exec-флагов пути указывается относительно директории выполнения команды. Для конфигурации пути передается из директории конфигурации.

Пример:

```bash
yfm -e @diplodoc/openapi-extension
yfm -e ./local-extension
```

||
||
`--watch, -w` | Запуск [watch-режима сборки](build.md#watch).
||
||
`--lang, --langs` | Настроить поддерживаемые языки.
||
||
`--output-format, -f` | Формат файлов итоговой сборки. Доступные варианты: `html`, `md`. По умолчанию `html`.
||
||
`--allow-html` | Разрешить использование HTML в markdown-файлах.
||
||
`--sanitize-html` | Включить очистку HTML.
||
||
`--allow-custom-resources`
|
Разрешить загрузку пользовательских ресурсов на статически сгенерированные страницы.
||
||
`--static-content`
|
Собирать html-контент статьи как часть вёрстки. По умолчанию, он находится в js-объекте и вставляется в страницу на этапе отрисовки в браузере.
||
||
`--theme <color>` | Подключить темизатор. Подробнее - [здесь](../../style/theme.md).
||
||
`--ignore`
|
Не обрабатывать пути, соответствующие шаблону.

Пример:

```bash
build -i ./input -o ./output --ignore *.bad.md
```

или

```bash
build -i ./ -o ./build --ignore ./build
```

||
||
`--ignore-stage`
|
Не обрабатывать файлы оглавления с указанным уровнем. По умолчанию `["skip"]`.
||
||
`--template <value>`
|
Выбрать режим работы [шаблонов](../../syntax/vars.md). По умолчанию включен текстовый режим, который игнорирует блоки кода. Используйте `all` или `code`, для обработки блоков кода.

Возможные значения:

- `text`;

- `all`;

- `code`.
||
||
`--template-vars`
|
Переключить режим обработки [переменных](../../syntax/vars.md#subtitudes) в двойных фигурных скобках. По умолчанию включено.
||
||
`--template-conditions`
|
Переключить режим обработки [условных операторов](../../syntax/vars.md#conditions). По умолчанию включено.
||
||
`--vcs` | Включить получение данных из vcs. Этот параметр отвечает только за включение/выключение.
У каждого vcs коннектора собственная конфигурация. Она должна быть описана в документации коннектора.
||
||
`--mtimes` | Включить отображение даты изменения статьи. Данные берутся из vcs.
||
||
`--authors` | Включить отображение автора статьи. Данные берутся из vcs.
||
||
`--contributors` | Включить отображение контрибьюторов в статью. Данные берутся из vcs.
||
||
`--lint` | Подключить [файл линтера](../../project/lint.md). По умолчанию включено. Чтобы отключить, используйте `--no-lint`.
||
||
`--llms` | Включить [генерацию файлов llms.txt и llms-full.txt](../../guides/llms.md) для проекта.
||
||
`--llms-full-max-size <value>` | Ограничение на размер генерируемого файла llms-full.txt одной статьи, в случае превышения которого наполнение файла данными останавливается с кодом ##YFM022## (сборка не завершается ошибкой). Принимает числа и строки вида: ##1024##, ##512K##, ##2M##.
||
||
`--changelogs` | Бета-функция. Переключить обработку синтаксиса экспериментального списка изменений.
||
||
 `--single-page` | Сборка проекта в виде одного HTML-файла. Подробнее в разделе [Одностраничная сборка](./singlepage.md).
||
||
 `--output-format <value>` | Формат генерации. По умолчанию HTML, но можно настроить [сборку в YFM](build.md#yfm).
||
||
 `--apply-presets` | Подставлять значения переменных из пресетов при сборке в YFM.
||
||
 `--remove-hidden-toc-items` | Удалять скрытые страницы из результата сборки. По умолчанию выключено.
||
||
 `--skip-html-extension` | Удалять из внутренних ссылок на статьи расширение `.html` и имя файла `index.html`.

{% note info %}

Для корректного открытия ссылок без расширений необходимо настроить веб-сервер на обработку путей без `.html` и `index.html` на конце.

{% endnote %}
||
||
 `--add-map-file` | Добавлять создание file.json со всеми путями к документации. По умолчанию выключено.
||
||
 `--build-stats` {#build-stats-flag} | Записать рядом с output файл `yfm-build-stats.json` с метриками сборки (длительность, счётчики страниц и ассетов, размер артефактов). Подробнее — в [Статистика сборки](build.md#build-stats). По умолчанию включено для сборок `--output-format=md`, для остальных — выключено. Чтобы отключить, используйте `--no-build-stats`.
||
||
 `--build-content` {#build-content-flag} | Записать рядом с output файл `yfm-build-content.json` с хешами содержимого каждого файла и зависимостями `страница → ассеты`. Используется внешними инструментами (переиндексация поиска, уведомления об изменениях) для вычисления набора страниц, изменившихся между двумя сборками. Подробнее — в [Карта содержимого сборки](build.md#build-content). По умолчанию включено для сборок `--output-format=md`, для остальных — выключено. Чтобы отключить, используйте `--no-build-content`.
||
||
 `--sitemap` {#sitemap-flag} | Генерировать в выходной директории сборки файл `sitemap.xml` с абсолютными ссылками на страницы. Работает только для HTML-сборок и требует флага ##--base-href <url>##. Страницы, помеченные `noIndex`, в файл не попадают. Чтобы отключить, используйте `--no-sitemap`.
||
||
 `--disable-csp` | Отключить добавление мета-тега [Content-Security-Policy](../../guides/csp.md#disable-csp) в сгенерированные HTML-страницы. По умолчанию включено.
||
||
 `--max-asset-size <value>` {#max-asset-size} | Ограничение на размер ассета в проекте, в случае превышения которого сборка завершается с ошибкой ##YFM013##. Принимает числа и строки вида: ##1024##, ##512K##, ##2M##.

Если указать ##0##, то проверка размера ассетов выполняться не будет.

По умолчанию ##64M##.
||
||
 `--max-html-size <value>` {#max-html-size} | Ограничение на размер html-контента одной статьи, в случае превышения которого сборка завершается с ошибкой ##YFM012##. Принимает числа и строки вида: ##1024##, ##512K##, ##2M##.

Работает только при сборке `--output-format html`.

По умолчанию ##42M##, максимальное значение ##96M##.
||
||
 `--max-inline-svg-size <value>` {#max-inline-svg-size} | Максимально допустимый размер svg-изображения, при котором оно [инлайнится в контент статьи](../../syntax/media.md#img-inline) автоматически. В случае превышения размера svg-изображение вставляется через тег ##\<img\>##. Принимает числа и строки вида: ##1024##, ##512K##, ##2M##.

Если указать ##0##, то svg-изображения не будут инлайниться при сборке.

По умолчанию ##2M##, максимальное значение ##16M##.
||
||
 `--max-openapi-include-size <value>` {#max-openapi-include-size} | Максимально допустимый размер файла, собранного из [OpenAPI-спецификации](./../../guides/openapi.md). В случае превышения размера файла, из него вырезается json-схема. Принимает числа и строки вида: ##1024##, ##512K##, ##2M##.

Если указать ##0##, ограничения нет.
||
||
 `--multiline-term-definitions` {#multiline-term-definitions} | Включает поддержку множественных переносов строк во [всплывающих подсказках](../../syntax/term.md).
||
||
 `--version` | Текущая версия.
||
|#

Чтобы ознакомиться с полным списком ключей, выполните команду `yfm build --help`.

Параметры сборки имеют **более высокий приоритет**, чем параметры из [файла настроек проекта](../../settings.md).

Если к параметру-флагу, _включающему_ определенное поведение, добавить префикс `no-`, он начнёт **отключать** это поведение. Например, параметр `--no-llms` _отключает_ сборку `llms.txt`, `--no-allow-html` _запрещает_ использование HTML в markdown-файлах и так далее.
