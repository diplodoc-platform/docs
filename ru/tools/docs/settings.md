...
||
||
 `--max-asset-size` {#max-asset-size} | Ограничение на размер ассета в проекте, в случае превышения которого сборка завершается с ошибкой ##YFM013##. Принимает числа и строки вида: ##1024##, ##512K##, ##2M##.
...
||
||
 `--max-html-size` {#max-html-size} | Ограничение на размер html-контента одной статьи, в случае превышения которого сборка завершается с ошибкой ##YFM012##. Принимает числа и строки вида: ##1024##, ##512K##, ##2M##.
...
||
||
 `--max-inline-svg-size` {#max-inline-svg-size} | Максимально допустимый размер svg-изображения, при котором оно [инлайнится в контент статьи](../../syntax/media.md#img-inline) автоматически. В случае превышения размера svg-изображение вставляется через тег ##\<img\>##. Принимает числа и строки вида: ##1024##, ##512K##, ##2M##.
...
||
||
 `--max-openapi-include-size` {#max-openapi-include-size} | Максимально допустимый размер файла, собранного из [OpenAPI-спецификации](./../../guides/openapi.md). В случае превышения размера файла, из него вырезается json-схема. Принимает числа и строки вида: ##1024##, ##512K##, ##2M##.
