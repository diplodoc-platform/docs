...
Библиотека включает шаблоны и функции для создания различных типов диаграмм, например: блок-схемы, графы, деревья, диаграммы Ганта, mind-карты, C4-диаграммы.
...
В подготовленной среде выполнения Mermaid отключена опция `startOnLoad`, чтобы точно контролировать шаг рендеринга. Затем добавляется глобальный обратный вызов `mermaidJsonp` для обработки загрузки Mermaid.
