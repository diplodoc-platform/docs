...
Liquid — это язык шаблонов. Diplodoc использует Liquid-совместимый синтаксис для динамической генерации контента: `{{ переменная }}` для подстановки значений и `{% if %}`, `{% for %}` для условий и циклов.
