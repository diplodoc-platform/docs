...
Сборщик умеет распараллеливать обработку страниц по нескольким потокам. Режим выключен по умолчанию и включается ключом [`--jobs`](../../tools/docs/settings.md).

| Ключ запуска | Описание |
| --- | --- |
| `--jobs, -j [число]` | Собирать в указанное число потоков. Если число не передано, используется `количество ядер - 1`. При значении `1` и меньше многопоточность не включается. |
| `--worker-max-old-space <МБ>` | Лимит памяти (old space) для каждого рабочего потока. |
...
* **Откат к однопоточной сборке.** Если рабочие потоки не успевают инициализироваться за 30 секунд, сборка молча продолжается в один поток. В логах будет строка `Threads setup timed out`.
* **Порядок записи файлов не определён.** Один и тот же файл может быть записан разными путями обработки: как страница из оглавления и как включаемый файл. В однопоточной сборке порядок стабильный, в многопоточной - нет. Поэтому все пути записи обязаны давать одинаковый результат: пишите файл целиком, со всеми метаданными, а не дописывайте его частями.
* Выигрыш от многопоточности заметен на задачах, упирающихся в процессор, - разбор и рендер большого количества страниц. На проектах в несколько десятков страниц накладные расходы на запуск потоков могут превысить выгоду.

## Что почитать дальше {#see-also}

* [Принципы разработки расширений](./core-concepts.md)
* [Сервисы Diplodoc](./services.md)
* [Параметры сборки документации](../../tools/docs/settings.md)
