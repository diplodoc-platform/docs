...
1. Заполните поле **Спецификация**: можно воспользоваться вашей OpenAPI спецификацией или примером.
    ...
    1\. В `x-real-host:` задается адрес домена.
    2\. В `x-docs-proxy-base:` задается каталог, на котором будет размещаться документация.
    3\. В `x-docs-project-name:` задается название проекта.
    
    Для настройки служебных заголовков укажите:||
