...
Сервис Быстрый старт («Quick Start») позволяет создать репозиторий и привязать его к внешнему сервису Diplodoc.
...
1. Откройте **Все сервисы** → **API Gateway**.
    ...
    API Gateway — упрощенный сервер в Yandex Cloud для обслуживания внешних запросов. Настраивается с помощью OpenAPI спецификации.
