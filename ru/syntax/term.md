...
В определении термина по умолчанию можно использовать **только одиночные переносы строки**. Включить поддержку множественных переносов строки в определениях можно одним из способов:

* выполнить сборку с параметром `--multiline-term-definitions`;
* добавить `content: multilineTermDefinitions: true` в [файл `.yfm`](../settings.md).
