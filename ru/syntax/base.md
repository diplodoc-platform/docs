...
`{red}(Цветной текст)` | {red}(Цветной текст)
...
У них есть ограничения: они не работают с якорями и не отображаются в _оглавлении страницы_.
...
Комментарии — элементы разметки, которые не отображаются в собранном файле. Они используются, чтобы хранить в исходном тексте информацию для SEO или авторов документа.
...
Чтобы добавить в результирующую HTML-разметку дополнительные атрибуты (например `class` или `id`), используйте синтаксис [markdown-it-attrs](https://github.com/arve0/markdown-it-attrs/blob/master/README.md). Этот плагин включен в YFM по умолчанию.
