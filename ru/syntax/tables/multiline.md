...
Широкие таблицы удобно открывать в модальном окне. В многострочных таблицах это реализуется с помощью атрибута `{wide-content title="название таблицы"}`. Атрибут добавляется сразу после символов, закрывающих таблицу `|#`.
