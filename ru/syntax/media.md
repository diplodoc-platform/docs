...
```
  ...
  * `width=100`, `height=100` — размер изображения. Необязательные параметры.
    ...
    Если вы хотите сохранить оригинальное соотношение сторон изображения, укажите **только** один из параметров — ширину **или** высоту.
...
## Галерея изображений и видео {#gallery}

По умолчанию, изображения и видео открываются в галерее по клику для полноразмерного просмотра и переключение между объектами в одной статье. _Изображение_ не будет открываться в галерее при выполнении **всех** условий из списка:

- изображение находится в одной строке с текстом или ширина и высота не превышают 150px;
- у изображения _не указан_ атрибут `{gallery-src}`;
- у изображения указан атрибут `{gallery=false}`;
- в статье отключен просмотр в галерее с помощью `interface: gallery: false` в метаданных страницы;
- в [конфигурации проекта](../settings.md#gallery) отключен просмотр в галерее с помощью `interface: gallery: false`.

Можно указать ссылку на версию изображения для просмотра в галерее с помощью атрибута `gallery-src`, например, чтобы в статье по умолчанию отображался более лёгкий файл, а в просмотрщике — полная версия.
...
В статье отображается изображение `mountain-tiny.jpg` (23Kb), в галерее — `mountain.jpg` (228Kb).
