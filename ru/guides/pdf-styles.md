...
При сборке PDF каждая статья попадает в свою обертку — блок `.pdf-page-wrapper` внутри `main.yfm`. Обертки идут друг за другом в том же порядке, что и статьи в оглавлении, и каждая начинается с новой страницы. Поэтому, если нужно стилизовать статью целиком, удобнее всего обращаться именно к этой обертке.
...
Использовать селектор по атрибуту надежнее, так как он не сломается, если имя класса изменится в будущих версиях сборки.
...
Пример оформления заголовков всех статей в документе цветом и отделение от текста тонкой линией снизу:
...
Чтобы якоря разных статей не конфликтовали в одном документе, к каждому `id` добавляется префикс с именем страницы: `id="anchor"` превращается в `id="page1_anchor"`. Селекторы по конкретному `id` в PDF не работают.

## Разрывы страниц { #page-breaks }

Разрывами управляют свойства `page-break-before`, `page-break-after` и `page-break-inside`. Часть таких правил Diplodoc применяет по умолчанию. Учитывайте их при написании своих стилей. Например, разрыв страницы не исчезнет, потому что его дублирует другое встроенное правило.
