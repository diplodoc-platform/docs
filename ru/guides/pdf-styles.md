...
- основной текст 15px;
- заголовки 17px до 32px.

При печати браузер переводит пиксели в пункты и дополнительно применяет масштаб 0,85. Поэтому на бумаге текст получается мельче, чем в CSS.
...
1. Соберите проект с флагами `--pdf-debug` и `--allow-custom-resources`:
    ...
    Флаг `--pdf-debug` дополнительно создает HTML-версии титульных и закрывающих страниц. Подробнее в разделе [Создание PDF из документации](generate-pdf.md#build).
