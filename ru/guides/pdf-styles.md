...
Без флага `--allow-custom-resources` блок `resources.style` игнорируется целиком. В результате стили не попадут в итоговую сборку.
...
Этот класс больше нигде не используется, поэтому он служит основным способом отделить правила для PDF от общих правил проекта:
...
Титульные страницы, статьи и закрывающие страницы используют один класс `.pdf-page-wrapper` и различаются только положением в дереве: титульные страницы — прямые потомки `body`, статьи лежат внутри `main.yfm`. Селектор `.pdf-page-wrapper` без дочернего комбинатора `>` применится ко всем трем.

{% endnote %}

### Титульные и закрывающие страницы { #start-end-pages }

[Титульные и закрывающие страницы](generate-pdf.md#start-pages) задаются в блоках `pdf.startPages` и `pdf.endPages` файла `toc.yaml`.

Титульные страницы адресуются по положению в дереве, закрывающие — по обертке `.pdf-ending-pages`:
...
Оглавление — это отдельная страница внутри PDF, которую генератор создает из файла `toc.yaml` и размещает между титульными страницами и основным контентом. С боковым меню сайта оно не связано.
...
- пункт со ссылкой на статью — это `<a>`, название раздела, объединяющего группу статей, — `<span>`;
...
Не путайте оглавление с закладками PDF — панелью навигации в программе для просмотра. Закладки формируются отдельным механизмом и стилями не управляются.

{% endnote %}

### Статьи { #articles }

Каждая статья — это отдельная обертка `.pdf-page-wrapper` внутри `main.yfm`. Одна обертка соответствует одной статье, статьи идут в том же порядке, что и в оглавлении, и каждая начинается с новой страницы.

У всех оберток есть атрибут `data-page-break="true"` — его можно использовать как альтернативный селектор:
...
Все статьи склеиваются в один документ, поэтому уровни заголовков понижаются на единицу:

| В Markdown | На сайте | В PDF |
|---|---|---|
...
В PDF нет ни одного тега `h1` — селекторы на `h1` не сработают. Заголовок статьи — это `h2` с атрибутом `data-original-article`.
...
Вторая особенность — идентификаторы. Чтобы якоря разных статей не конфликтовали в одном документе, к каждому `id` добавляется префикс с именем страницы: `id="anchor"` превращается в `id="page1_anchor"`. Селекторы по конкретному `id` в PDF не работают.

## Разрывы страниц { #page-breaks }

Разрывами управляют свойства `page-break-before`, `page-break-after` и `page-break-inside`. Часть правил Diplodoc задает сам — их важно учитывать, чтобы не бороться с собственной версткой.
...
| `h2[data-original-article]` | `page-break-before: always` | то же, дублирующий механизм |
...
Набор правил пополняется по мере развития генератора PDF — перед версткой сложных документов сверяйтесь с этой таблицей.
...
CSS для PDF пишется в пикселях — это обычная веб-страница. Базовые размеры контента заданы переменными `yfm.css`: основной текст — 15px, заголовки — от 17px до 32px.

При печати браузер переводит пиксели в пункты и дополнительно применяет масштаб 0,85. Поэтому на бумаге текст мельче, чем указано в CSS: `font-size: 15px` дает около 9,6 pt.

{% note info %}

Размер на бумаге = значение в CSS × 0,75 × 0,85.

Подбирая кегль для печати, проверяйте результат на распечатке или в предпросмотре печати.

{% endnote %}

## Порядок применения стилей { #cascade }

Корректно написанное правило может не сработать из-за порядка подключения стилей. В PDF-документе стили подключаются в таком порядке (ниже — сильнее):
...
Чаще всего пользовательские правила перебивают стили генератора, которые адаптируют контент к печати:
...
Генератор создает промежуточный файл `pdf-source.html` — это ровно тот документ, который печатается в PDF. Его можно открыть в браузере и исследовать через инструменты разработчика.
...
1. Соберите проект с флагами `--pdf-debug` и `--allow-custom-resources`:
    ...
    Флаг `--pdf-debug` дополнительно создает HTML-версии титульных и закрывающих страниц. Подробнее о сборке см. в разделе [Сборка](generate-pdf.md#build) (документ [Создание PDF из документации](generate-pdf.md)).
...
1. Откройте в браузере файл `docs-output/pdf/pdf-source.html`. В инструментах разработчика видно итоговую разметку, все подключенные стили и то, какое правило выиграло в каскаде.
