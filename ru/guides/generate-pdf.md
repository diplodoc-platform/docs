...
1. **Титульные страницы** {#start-pages}
    ...
    Чтобы сформировать титульные страницы, настройте блок `startPages` в `toc.yaml`:
...
Чтобы собрать PDF-версию документации:

1. [Установите пакет `@diplodoc/pdf-generator`](../settings.md#pdf).

1. В файле toc.yaml добавьте секцию `startPages` для формирования [титульной страницы](#start-pages).

1. Добавьте параметр в файл конфигурации `.yfm`:
