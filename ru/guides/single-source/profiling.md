...
Если текст размечен с помощью переменных, то в файле `presets.yaml` должны быть пресеты для каждого варианта профилирования. При сборке значения переменных берутся из пресета `default` и пресета, который указан в [конфигурационном файле `.yfm`](../../settings.md) в параметре `varsPreset`.
...
В [конфигурационный файл `.yfm`](../../settings.md) добавьте следующие параметры сборки:
