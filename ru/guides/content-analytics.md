...
В отчетах по Контенту данные собираются только по размеченным материалам. Для разметки материала рекомендуется использовать [OpenGraph](https://yandex.ru/support/metrica/publishers/open-graph/open-graph.html) — разметку, которая задаётся в файлах документации с расширением `md`, в блоке [front-matter](*front-matter).
