...
* `spec.renderMode` - определяет режим добавления json-схемы спецификации на страницу оглавления. Поддерживаемые значения:
    * `inline` (по умолчанию) — json-схема вставляется прямо на страницу. Если размер json-схемы превышает значение [##maxOpenapiIncludeInlineSize##](../settings.md#max-openapi-include-size) (по умолчанию равно 100Кб), режим автоматически переключается на `link`.
    * `link` — на страницу оглавления вставляется ссылка на файл json-схемы.
    * `hidden` — вставлка json-схемы не выполняется.
