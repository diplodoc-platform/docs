...
#|
|| **Название** | **Описание** | **Тип и значение по умолчанию** ||
|| `toc` | Скрывает TOC. Если не указан, TOC считается включённым. | `bool`<br>`true` ||
|| `search` | Скрывает поиск. Если не указан, поиск считается включённым. | `bool`<br>`true` ||
|| `feedback` | Скрывает фидбэк. Если не указан, фидбэк считается включённым. | `bool`<br>`true` ||
|| `toc-header` | Скрывает заголовок в TOC. Если не указан, заголовок считается включённым. | `bool`<br>`true` ||
