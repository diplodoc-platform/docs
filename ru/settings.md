...
|| `authors` | Включить отображение автора статьи. Берется из данных vcs. | `bool` или `object`<br>`false`
Параметры объекта:
...
|| `contributors` | Включить отображение контрибьюторов в статью. Берется из данных vcs. | `bool` или `object`<br>`false`
Параметры объекта:
- `enabled` - `bool` - Включить отображение автора статьи.
- `ignore` - `string[]` - Игнорировать авторов по паттернам перечисленным в списке.
