...
Некоторые параметры можно задать или переопределить через [ключи запуска сборки](./tools/docs/settings.md).
...
`false`
||
|| `sitemap` {#sitemap} | Генерировать файл `sitemap.xml` в выходной директории сборки. Файл содержит список страниц документации в виде абсолютных ссылок, которые строятся относительно параметра `baseHref`, поэтому задание `baseHref` обязательно. Страницы, помеченные `noIndex`, в файл не попадают. | `boolean`
...
Настройки отображения интерфейса. Все настройки из секции можно переопределять для отдельных статей, указывая их значения в [метаданных страниц](./project/meta.md#interface).
...
`true` ||
|| `feedback` | Скрывает форму обратной связи в боковом меню и в конце страницы. Если не указан, приём обратной связи включен. | `boolean`
...
> [Ajv схема файлов конфигурации Diplodoc](https://raw.githubusercontent.com/diplodoc-platform/ajv/refs/heads/master/src/json/yfm-schema.json)
