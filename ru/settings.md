...
| `bool` | `false` ||
|| `needToSanitizeHtml` | Параметр предназначен для очистки и фильтрации HTML-контента. Связанный параметр `sanitizeOptions` позволяет определить, какие HTML-теги, атрибуты, стили и другие элементы будут разрешены или запрещены в процессе очистки HTML-контента. | `bool` | `false` ||
