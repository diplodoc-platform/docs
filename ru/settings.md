...
Параметры выкладки собранной документации в [хранилище S3](https://cloud.yandex.ru/services/storage). Подробнее см. в разделе [Выкладка на S3](./tools/docs/publish-s3.md).

#|
|| **Название** | **Описание** | **Тип и значение по умолчанию** ||
|| `endpoint` | Эндпоинт для хранилища S3. | `string`

`https://s3.amazonaws.com` ||
|| `bucket` | Бакет. | `string`

— ||
|| `prefix` | Префикс для путей файлов.

Необязательный параметр. Может использоваться для передачи версии сборки — каждая сборка будет складываться в отдельную папку. | `string`

— ||
|| `region` | Регион S3-хранилища. | `string`
