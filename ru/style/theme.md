...
Управлять темой документации можно с помощью конфигурационного файла `theme.yaml` в корне проекта либо при сборке c помощью флага.
...
`hljs-background`| Фон блока кода.
...
Если одновременно заданы и файл конфигурации, и флаг `--theme`, то флаг переопределит параметр `base-brand` только в основной секции. Значения в секциях `light` и `dark` останутся без изменений.